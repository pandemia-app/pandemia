...
void main() {
  ...
  test("returns cached data if possible", () async {
    int httpCallsDone = 0;
    Favorite placeWithStats = new Favorite(
        name: "E.Leclerc Hyper Paris 19",
        address: "191 Boulevard Macdonald, 75019 Paris, France",
    ...
    // doing a second call to check if cached data is returned
    var cachedStats = await parser.getPopularTimes(placeWithStats);
    expect(cachedStats.hasData, true);
    // we got data two times, with one HTTP call if cache worked
    expect(httpCallsDone, 1);
  });
}
